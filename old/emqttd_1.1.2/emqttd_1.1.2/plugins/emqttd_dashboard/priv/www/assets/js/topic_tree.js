...
var wi = window, d = document, e = d.documentElement, 
...
function setup(tagID) {
	...
	vis = d3.select("#" + tagID).append("svg:svg").attr("style",
			"overflow: auto;").attr("width", w + m[1] + m[3]).attr("height",
...
function update(source) {
	...
	nodeEnter
			...
			.attr(
					"onmouseover",
					"evt.target.parentNode.getElementsByClassName('popup')[0].setAttribute('style','opacity: 1');")
			.attr(
					"onmouseout",
					"evt.target.parentNode.getElementsByClassName('popup')[0].setAttribute('style','opacity: 0');");

	var popup = nodeEnter.append("svg:g");
	popup.attr("class", "popup").style("opacity", 0).append("svg:rect").attr(
			"class", "popupRect").attr("x", 10).attr("y", 25).attr("rx", 5)
			.attr("ry", 5).attr("width", 500).attr("height", 20);

	popup.append("svg:text").attr("x", 15).attr("y", 35).attr("dy", ".35em")
			.attr("class", "popupTxt").text(function(d) {
				if (d.data) {
	...
	var nodeChange = node.selectAll("text.data").text(function(d) {
		if (d.data) {
			if (d.data.length > 50) {
				return d.data.substring(0, 50) + "...";
			} else {
	...
	var popupChange = node.selectAll("text.popupTxt").text(function(d) {
		if (d.data) {
			if (d.data.length > 100) {
				return d.data.substring(0, 100) + "...";
			} else {
	...
	// popup.select("rect.popupRect").attr("width",
	// popup.select("text.popupTxt").node().getComputedTextLength() + 20);
	...
	nodeUpdate.select("text.popup").style("fill-opacity", 1e-6);
...
function walk(parts, node, body) {
	if (parts.length != 0) {
		...
		if (node.children && node.children.length != 0) {
			...
			// console.log("done hidden loop - " + z + ", " +
