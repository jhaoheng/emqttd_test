...
register_auth_mod(SuperQuery) ->
    {ok, UserAuthQuery} = application:get_env(?APP, userauthquery),
    {ok, DeviceAuthQuery} = application:get_env(?APP, deviceauthquery),
    {ok, HashType}  = application:get_env(?APP, password_hash),
    AuthEnv = {SuperQuery, parse_query(UserAuthQuery), parse_query(DeviceAuthQuery), HashType},
